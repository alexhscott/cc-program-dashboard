...
const SB = supabase.createClient(SUPA_URL, SUPA_KEY, {
  auth: {
    persistSession: true,
    autoRefreshToken: false,       // don't auto-refresh — let inactivity timer handle expiry
...
async function flushSaveQueue(){
  ...
  if(error){
    setSaveStatus('Save failed: '+error.message, '#E1523D');
  } else {
...
async function loadPersistedData(){
  ...
  // Helper: run a Supabase query with a hard 8-second timeout
  ...
  try{
    // Load program data with its own independent timeout
    const progResult = await withTimeout(
      SB.from('program_data').select('year,month,section_id,region,field_id,value'),
      8000
    );

    if(progResult.error){
      setSaveStatus('Load error: '+progResult.error.message, '#E1523D');
      return false;
    }
    if(progResult.data && progResult.data.length){
      mergeAppData(progResult.data);
    }

    // Load graduate entries with its own independent timeout
    const gradResult = await withTimeout(
      SB.from('graduate_entries').select('*').order('entry_id'),
      8000
    );

    if(!gradResult.error && gradResult.data && gradResult.data.length){
      manualEntries = gradResult.data.map(r=>({
        id: r.entry_id,
    ...
    // Clear loading status so it never stays stuck
    setSaveStatus('', 'var(--text-muted)');
    return false;
  }
}
...
async function afterLogin(){
  ...
  // 4. Reload all visible entry fields with loaded values
  // Use a delay to ensure DOM is fully built before populating
  setTimeout(()=>{
    const activePanel = document.querySelector('.sub-panel.active');
    if(activePanel){
      const secId = activePanel.id.replace('panel-','');
      if(secId) loadSectionValues(secId);
    }
  }, 100);
}
...
function renderManualTable(){
  ...
  tb.innerHTML=manualEntries.map(e=>{
    ...
    return `<tr class="${cls}">
      ...
      <td>${status}</td>
      <td><button class="del-btn" onclick="removeGradEntry(${e.id})" title="Delete entry">×</button></td>
    </tr>`;
  }).join('');
}
...
(async function init(){
  // Always show login first — then check if a valid Supabase session exists
  showLogin();
