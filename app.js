...
function showApp(){
  ...
  document.querySelectorAll('.page').forEach(p=>{ if(p.id!=='page-login') p.style.display=''; });
...
function showLogin(){
  ...
  document.querySelectorAll('.page').forEach(p=>{ if(p.id!=='page-login') p.style.display='none'; });
  const badge = document.getElementById('user-badge');
  const logoutBtn = document.getElementById('logout-btn');
...
(async function init(){
  // Hide app — show login by default
  showLogin();

  // Check if there's an existing session (user already logged in)
  ...
  // No session → login page stays visible
})();