...
async function loadPersistedData(){
  ...
  try{
    // 5-second timeout — if Supabase tables don't exist yet, fail gracefully
    const timeout = new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),5000));

    const [progResult, gradResult] = await Promise.all([
      Promise.race([
    ...
    ]).catch(()=>[{data:null,error:'timeout'},{data:null,error:'timeout'}]);

    if(progResult?.data?.length){
      mergeAppData(progResult.data);
    }
    if(gradResult?.data?.length){
...
async function afterLogin(){
  ...
  // 4. Reload visible entry fields so inputs show saved values
...
function showPage(id){
  ...
  if(id==='entry'){
    ...
    // Small delay to let panels render before loading values
    setTimeout(()=>{
