...
function showApp(){
  ...
  // Don't touch individual page display — the active class system handles visibility
  const badge = document.getElementById('user-badge');
  const logoutBtn = document.getElementById('logout-btn');
  if(badge && _currentUser){ badge.textContent=_currentUser.email; badge.style.display=''; }
...
function showLogin(){
  ...
  // Remove active from all app pages
...
async function afterLogin(){
  ...
  // showPage('home') sets the active class AND calls buildTOC
  showPage('home');
}
...
function showPage(id){
  ...
  if(id==='home')buildTOC();
  if(id==='entry'){
    // Rebuild tabs filtered to current role's sections
...
(async function init(){
  // Hide app while we check auth state
  showLogin();

  // Listen for auth state changes (handles refresh, tab focus, token refresh)
  SB.auth.onAuthStateChange(async (event, session) => {
    if(event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED' || event === 'INITIAL_SESSION'){
      if(session){
        _currentUser = session.user;
        // Only run afterLogin if we're not already showing the app
        if(document.getElementById('page-login').classList.contains('hidden')) return;
        await afterLogin();
      }
  ...
  // Also do an immediate check so there's no flicker on page load
  const { data: { session } } = await SB.auth.getSession();
  if(session){
    _currentUser = session.user;
    await afterLogin();
  }
  // No session → login page stays visible (already shown by showLogin() above)
})();