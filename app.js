...
async function loadPersistedData(){
  ...
  // Hard 6-second wall clock timeout — if anything hangs, we bail and clear status
  const deadline = new Promise(resolve=>setTimeout(()=>resolve('timeout'), 6000));
  ...
  const run = async()=>{
    try{
      ...
      if(!error && data && data.length) mergeAppData(data);
