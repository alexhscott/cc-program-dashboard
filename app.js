...
const SB = supabase.createClient(SUPA_URL, SUPA_KEY, {
  auth: {
    ...
    storageKey: 'cc-dashboard-auth', // unique key so it doesn't clash
  }
});
...
function debouncePersist(){
  ...
  _saveTimer = setTimeout(()=>flushSaveQueue(), 1500);
}
...
async function flushSaveQueue(){
  ...
  const { error } = await SB
    .from('program_data')
    .upsert(rows, { onConflict: 'year,month,section_id,region,field_id' });
  _storageSaving = false;
  if(error){
    setSaveStatus('Save failed', '#E1523D');
  } else {
    const ts = new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
    setSaveStatus('Saved '+ts, '#FECB00');
    setTimeout(()=>setSaveStatus('','var(--text-muted)'),4000);
  }
}
...
function mergeAppData(rows){
  ...
  rows.forEach(r=>{
    sv(r.year, r.month, r.section_id, r.region, r.field_id, r.value);
  });
}
...
async function loadPersistedData(){
  ...
  setSaveStatus('Loading data…', 'var(--text-muted)');
  try{
    // Load program data
    ...
    if(!progErr && progRows && progRows.length){
      mergeAppData(progRows);
    }
    // Load graduate entries
    const { data: gradRows, error: gradErr } = await SB
      .from('graduate_entries')
      .select('*')
      .order('entry_id');
    if(!gradErr && gradRows && gradRows.length){
      manualEntries = gradRows.map(r=>({
        id: r.entry_id,
        first: r.first_name, last: r.last_name, mi: r.middle_initial||'',
        dog: r.dog_name, cat: r.category, region: r.region,
        date: r.graduation_date,
        matchClient: null, matchDog: null
      }));
    }
    setSaveStatus('', 'var(--text-muted)');
    return true;
  } catch(e){
    setSaveStatus('Load error', '#E1523D');
    return false;
  }
}
...
async function afterLogin(){
  ...
  showApp();
  buildEntryPage();
  showPage('home');   // render cards right away with seed data

  // 2. Load real data from Supabase in background
  await loadPersistedData();

  // 3. Refresh UI with live data
  if(manualEntries.length) renderManualTable();
  buildTOC();         // re-render cards with any loaded state
}
...
function setActiveRegion(region){
  ...
  // Activate the matching region panel in whichever section is currently visible
...
function activateSection(id){
  ...
  // Auto-jump to the globally selected region after a brief render delay
  setTimeout(()=>syncRegionTabToActive(id), 30);
}
...
function renderManualTable(){
  ...
  tb.innerHTML=manualEntries.map(e=>{
    ...
    return `<tr class="${cls}">
      ...
      <td>${status}</td>
      <td><button class="del-btn" onclick="removeGradEntry(${e.id})"></button></td>
    </tr>`;
  }).join('');
}
...
(function seedData(){
  ...
  // app_requests ~90-120/mo org-wide, added_waitlist ~6-10
  // waitlist breakdown sums to waitlist_total (computed)
  const appSeed = {
    'North Central': [{app_requests:18,added_waitlist:2,wl_facility:14,wl_hearing:8,wl_service:22,wl_medical_alert:6,wl_ptsd:9,wl_service_fac:5},{app_requests:21,added_waitlist:3,wl_facility:15,wl_hearing:9,wl_service:23,wl_medical_alert:6,wl_ptsd:10,wl_service_fac:5},{app_requests:19,added_waitlist:2,wl_facility:15,wl_hearing:9,wl_service:24,wl_medical_alert:7,wl_ptsd:10,wl_service_fac:6},{app_requests:22,added_waitlist:3,wl_facility:16,wl_hearing:10,wl_service:25,wl_medical_alert:7,wl_ptsd:11,wl_service_fac:6},{app_requests:20,added_waitlist:2,wl_facility:16,wl_hearing:10,wl_service:26,wl_medical_alert:8,wl_ptsd:11,wl_service_fac:7}],
    'Northeast':     [{app_requests:22,added_waitlist:3,wl_facility:18,wl_hearing:11,wl_service:28,wl_medical_alert:8,wl_ptsd:12,wl_service_fac:7},{app_requests:24,added_waitlist:3,wl_facility:19,wl_hearing:12,wl_service:29,wl_medical_alert:8,wl_ptsd:12,wl_service_fac:7},{app_requests:23,added_waitlist:3,wl_facility:19,wl_hearing:12,wl_service:30,wl_medical_alert:9,wl_ptsd:13,wl_service_fac:8},{app_requests:25,added_waitlist:4,wl_facility:20,wl_hearing:13,wl_service:31,wl_medical_alert:9,wl_ptsd:13,wl_service_fac:8},{app_requests:24,added_waitlist:3,wl_facility:20,wl_hearing:13,wl_service:32,wl_medical_alert:10,wl_ptsd:14,wl_service_fac:9}],
    'Northwest':     [{app_requests:16,added_waitlist:2,wl_facility:12,wl_hearing:7,wl_service:18,wl_medical_alert:5,wl_ptsd:8,wl_service_fac:4},{app_requests:17,added_waitlist:2,wl_facility:13,wl_hearing:7,wl_service:19,wl_medical_alert:5,wl_ptsd:8,wl_service_fac:4},{app_requests:16,added_waitlist:2,wl_facility:13,wl_hearing:8,wl_service:19,wl_medical_alert:6,wl_ptsd:9,wl_service_fac:5},{app_requests:18,added_waitlist:2,wl_facility:14,wl_hearing:8,wl_service:20,wl_medical_alert:6,wl_ptsd:9,wl_service_fac:5},{app_requests:17,added_waitlist:2,wl_facility:14,wl_hearing:8,wl_service:21,wl_medical_alert:6,wl_ptsd:10,wl_service_fac:5}],
    'South Central': [{app_requests:20,added_waitlist:2,wl_facility:16,wl_hearing:9,wl_service:24,wl_medical_alert:7,wl_ptsd:10,wl_service_fac:6},{app_requests:22,added_waitlist:3,wl_facility:17,wl_hearing:10,wl_service:25,wl_medical_alert:7,wl_ptsd:11,wl_service_fac:6},{app_requests:21,added_waitlist:2,wl_facility:17,wl_hearing:10,wl_service:26,wl_medical_alert:8,wl_ptsd:11,wl_service_fac:7},{app_requests:23,added_waitlist:3,wl_facility:18,wl_hearing:11,wl_service:27,wl_medical_alert:8,wl_ptsd:12,wl_service_fac:7},{app_requests:22,added_waitlist:2,wl_facility:18,wl_hearing:11,wl_service:28,wl_medical_alert:9,wl_ptsd:12,wl_service_fac:8}],
    'Southeast':     [{app_requests:19,added_waitlist:2,wl_facility:15,wl_hearing:9,wl_service:22,wl_medical_alert:6,wl_ptsd:10,wl_service_fac:5},{app_requests:21,added_waitlist:3,wl_facility:16,wl_hearing:9,wl_service:23,wl_medical_alert:7,wl_ptsd:10,wl_service_fac:6},{app_requests:20,added_waitlist:2,wl_facility:16,wl_hearing:10,wl_service:24,wl_medical_alert:7,wl_ptsd:11,wl_service_fac:6},{app_requests:22,added_waitlist:3,wl_facility:17,wl_hearing:10,wl_service:25,wl_medical_alert:8,wl_ptsd:11,wl_service_fac:7},{app_requests:21,added_waitlist:2,wl_facility:17,wl_hearing:10,wl_service:26,wl_medical_alert:8,wl_ptsd:12,wl_service_fac:7}],
    'Southwest':     [{app_requests:17,added_waitlist:2,wl_facility:13,wl_hearing:8,wl_service:20,wl_medical_alert:6,wl_ptsd:9,wl_service_fac:5},{app_requests:19,added_waitlist:2,wl_facility:14,wl_hearing:8,wl_service:21,wl_medical_alert:6,wl_ptsd:9,wl_service_fac:5},{app_requests:18,added_waitlist:2,wl_facility:14,wl_hearing:9,wl_service:22,wl_medical_alert:7,wl_ptsd:10,wl_service_fac:5},{app_requests:20,added_waitlist:2,wl_facility:15,wl_hearing:9,wl_service:23,wl_medical_alert:7,wl_ptsd:10,wl_service_fac:6},{app_requests:19,added_waitlist:2,wl_facility:15,wl_hearing:9,wl_service:24,wl_medical_alert:8,wl_ptsd:11,wl_service_fac:6}],
    'PSFO':          [{app_requests:8,added_waitlist:1,wl_facility:6,wl_hearing:3,wl_service:9,wl_medical_alert:2,wl_ptsd:4,wl_service_fac:2},{app_requests:9,added_waitlist:1,wl_facility:7,wl_hearing:4,wl_service:10,wl_medical_alert:3,wl_ptsd:4,wl_service_fac:2},{app_requests:8,added_waitlist:1,wl_facility:7,wl_hearing:4,wl_service:10,wl_medical_alert:3,wl_ptsd:5,wl_service_fac:3},{app_requests:10,added_waitlist:1,wl_facility:7,wl_hearing:4,wl_service:11,wl_medical_alert:3,wl_ptsd:5,wl_service_fac:3},{app_requests:9,added_waitlist:1,wl_facility:8,wl_hearing:4,wl_service:11,wl_medical_alert:3,wl_ptsd:5,wl_service_fac:3}],
  };
  Object.entries(appSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'applicants',r,f,v,true)));
  });

  // ── Graduates ──
  const gradSeed = {
    'North Central': [{adi_tests:4,checkins_6mo:12,live_meetings:8,touches:34},{adi_tests:5,checkins_6mo:13,live_meetings:9,touches:36},{adi_tests:4,checkins_6mo:14,live_meetings:9,touches:38},{adi_tests:5,checkins_6mo:15,live_meetings:10,touches:40},{adi_tests:5,checkins_6mo:15,live_meetings:10,touches:41}],
    'Northeast':     [{adi_tests:5,checkins_6mo:15,live_meetings:10,touches:42},{adi_tests:6,checkins_6mo:16,live_meetings:11,touches:44},{adi_tests:5,checkins_6mo:17,live_meetings:11,touches:46},{adi_tests:6,checkins_6mo:18,live_meetings:12,touches:48},{adi_tests:6,checkins_6mo:18,live_meetings:12,touches:50}],
    'Northwest':     [{adi_tests:3,checkins_6mo:10,live_meetings:7,touches:28},{adi_tests:4,checkins_6mo:11,live_meetings:7,touches:30},{adi_tests:3,checkins_6mo:11,live_meetings:8,touches:31},{adi_tests:4,checkins_6mo:12,live_meetings:8,touches:33},{adi_tests:4,checkins_6mo:12,live_meetings:8,touches:34}],
    'South Central': [{adi_tests:4,checkins_6mo:13,live_meetings:9,touches:36},{adi_tests:5,checkins_6mo:14,live_meetings:9,touches:38},{adi_tests:4,checkins_6mo:14,live_meetings:10,touches:40},{adi_tests:5,checkins_6mo:15,live_meetings:10,touches:42},{adi_tests:5,checkins_6mo:15,live_meetings:11,touches:43}],
    'Southeast':     [{adi_tests:4,checkins_6mo:12,live_meetings:8,touches:34},{adi_tests:5,checkins_6mo:13,live_meetings:9,touches:36},{adi_tests:4,checkins_6mo:14,live_meetings:9,touches:38},{adi_tests:5,checkins_6mo:14,live_meetings:10,touches:39},{adi_tests:5,checkins_6mo:15,live_meetings:10,touches:41}],
    'Southwest':     [{adi_tests:3,checkins_6mo:11,live_meetings:7,touches:30},{adi_tests:4,checkins_6mo:12,live_meetings:8,touches:32},{adi_tests:4,checkins_6mo:12,live_meetings:8,touches:33},{adi_tests:4,checkins_6mo:13,live_meetings:8,touches:35},{adi_tests:4,checkins_6mo:13,live_meetings:9,touches:36}],
    'PSFO':          [{adi_tests:2,checkins_6mo:5,live_meetings:3,touches:14},{adi_tests:2,checkins_6mo:5,live_meetings:3,touches:15},{adi_tests:2,checkins_6mo:6,live_meetings:4,touches:16},{adi_tests:3,checkins_6mo:6,live_meetings:4,touches:17},{adi_tests:3,checkins_6mo:6,live_meetings:4,touches:17}],
  };
  Object.entries(gradSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'graduates',r,f,v,true)));
  });

  // ── Dog Placements ──
  // campus, credited, ended, special — realistic monthly placement numbers
  const dpRegions = ['North Central','Northeast','Northwest','South Central','Southeast','Southwest'];
  const dpData = {
    campus:{
      'North Central':[{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:6,medical_alert:1,service_fac:2,ptsd:2},{facility:4,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3}],
      'Northeast':     [{facility:4,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3},{facility:5,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3},{facility:5,hearing:3,service:8,medical_alert:3,service_fac:3,ptsd:4},{facility:5,hearing:3,service:8,medical_alert:3,service_fac:4,ptsd:4}],
      'Northwest':     [{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:2,service:4,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3}],
      'South Central': [{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:4}],
      'Southeast':     [{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3}],
      'Southwest':     [{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3},{facility:3,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3}],
      'PSFO':          [{facility:1,hearing:1,service:2,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:1,hearing:1,service:3,medical_alert:1,service_fac:1,ptsd:1},{facility:2,hearing:1,service:3,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:1,service:3,medical_alert:1,service_fac:2,ptsd:2}],
    },
    credited:{
      'North Central': [{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:2,service:4,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3},{facility:3,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3}],
      'Northeast':     [{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:3},{facility:4,hearing:3,service:7,medical_alert:2,service_fac:3,ptsd:4}],
      'Northwest':     [{facility:2,hearing:1,service:3,medical_alert:1,service_fac:1,ptsd:1},{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:2,service:4,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:4,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:2}],
      'South Central': [{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3},{facility:4,hearing:2,service:6,medical_alert:2,service_fac:2,ptsd:3}],
      'Southeast':     [{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:2,service:4,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:3}],
      'Southwest':     [{facility:2,hearing:1,service:3,medical_alert:1,service_fac:1,ptsd:1},{facility:2,hearing:1,service:4,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:2,service:4,medical_alert:1,service_fac:2,ptsd:2},{facility:3,hearing:2,service:4,medical_alert:2,service_fac:2,ptsd:2},{facility:3,hearing:2,service:5,medical_alert:2,service_fac:2,ptsd:2}],
    },
    ended:{
      'North Central': [{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:1},{facility:1,hearing:1,service:1,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:2,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2}],
      'Northeast':     [{facility:1,hearing:1,service:2,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:2,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:1,service:3,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:1,service:3,medical_alert:1,service_fac:2,ptsd:2}],
      'Northwest':     [{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:1},{facility:1,hearing:0,service:1,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:1,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2}],
      'South Central': [{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:2,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:1,service:3,medical_alert:1,service_fac:1,ptsd:2}],
      'Southeast':     [{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:1},{facility:1,hearing:1,service:1,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2},{facility:2,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2}],
      'Southwest':     [{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:0},{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:1},{facility:1,hearing:1,service:1,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:2}],
      'PSFO':          [{facility:0,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:0},{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:0},{facility:1,hearing:0,service:1,medical_alert:0,service_fac:0,ptsd:1},{facility:1,hearing:1,service:1,medical_alert:0,service_fac:1,ptsd:1},{facility:1,hearing:1,service:2,medical_alert:1,service_fac:1,ptsd:1}],
    },
    special:{
      'North Central': [{offsite:1,successors:0,veterans:2,minors:1},{offsite:1,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:3,minors:2},{offsite:2,successors:1,veterans:3,minors:2}],
      'Northeast':     [{offsite:2,successors:1,veterans:3,minors:1},{offsite:2,successors:1,veterans:3,minors:2},{offsite:2,successors:1,veterans:4,minors:2},{offsite:3,successors:2,veterans:4,minors:2},{offsite:3,successors:2,veterans:4,minors:2}],
      'Northwest':     [{offsite:1,successors:0,veterans:1,minors:1},{offsite:1,successors:0,veterans:2,minors:1},{offsite:1,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:2}],
      'South Central': [{offsite:1,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:3,minors:2},{offsite:2,successors:1,veterans:3,minors:2},{offsite:2,successors:2,veterans:3,minors:2}],
      'Southeast':     [{offsite:1,successors:0,veterans:2,minors:1},{offsite:1,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:3,minors:2},{offsite:2,successors:1,veterans:3,minors:2}],
      'Southwest':     [{offsite:1,successors:0,veterans:1,minors:1},{offsite:1,successors:0,veterans:2,minors:1},{offsite:1,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:1},{offsite:2,successors:1,veterans:2,minors:2}],
      'PSFO':          [{offsite:0,successors:0,veterans:1,minors:0},{offsite:1,successors:0,veterans:1,minors:0},{offsite:1,successors:0,veterans:1,minors:1},{offsite:1,successors:1,veterans:1,minors:1},{offsite:1,successors:1,veterans:2,minors:1}],
    },
  };
  Object.entries(dpData).forEach(([ps,regions])=>{
    Object.entries(regions).forEach(([r,months])=>{
      months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'dog_placements',r,ps+'_'+f,v,true)));
    });
  });

  // ── Professional Training Dogs ──
  const ptSeed = {
    'North Central': [{sem1:4,sem2:5,sem3:3},{sem1:4,sem2:5,sem3:3},{sem1:5,sem2:6,sem3:4},{sem1:5,sem2:6,sem3:4},{sem1:5,sem2:6,sem3:4}],
    'Northeast':     [{sem1:5,sem2:6,sem3:4},{sem1:5,sem2:6,sem3:4},{sem1:6,sem2:7,sem3:5},{sem1:6,sem2:7,sem3:5},{sem1:6,sem2:8,sem3:5}],
    'Northwest':     [{sem1:3,sem2:4,sem3:2},{sem1:3,sem2:4,sem3:3},{sem1:4,sem2:4,sem3:3},{sem1:4,sem2:5,sem3:3},{sem1:4,sem2:5,sem3:3}],
    'South Central': [{sem1:4,sem2:5,sem3:3},{sem1:4,sem2:5,sem3:3},{sem1:5,sem2:5,sem3:4},{sem1:5,sem2:6,sem3:4},{sem1:5,sem2:6,sem3:4}],
    'Southeast':     [{sem1:4,sem2:4,sem3:3},{sem1:4,sem2:5,sem3:3},{sem1:4,sem2:5,sem3:3},{sem1:5,sem2:5,sem3:4},{sem1:5,sem2:6,sem3:4}],
    'Southwest':     [{sem1:3,sem2:4,sem3:2},{sem1:3,sem2:4,sem3:3},{sem1:4,sem2:4,sem3:3},{sem1:4,sem2:5,sem3:3},{sem1:4,sem2:5,sem3:3}],
    'PSFO':          [{sem1:2,sem2:2,sem3:1},{sem1:2,sem2:3,sem3:1},{sem1:2,sem2:3,sem3:2},{sem1:3,sem2:3,sem3:2},{sem1:3,sem2:3,sem3:2}],
  };
  Object.entries(ptSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'pro_training',r,f,v,true)));
  });

  // ── Release Dogs ──
  const relSeed = {
    'North Central': [{to_therapy:1,agency_transfer:0,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:1,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:2,all_other:3}],
    'Northeast':     [{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:2,agency_transfer:1,prior_matric:1,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:2,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:2,all_other:4},{to_therapy:2,agency_transfer:2,prior_matric:2,all_other:4}],
    'Northwest':     [{to_therapy:1,agency_transfer:0,prior_matric:1,all_other:1},{to_therapy:1,agency_transfer:0,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:3}],
    'South Central': [{to_therapy:1,agency_transfer:0,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:1,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:2,all_other:3}],
    'Southeast':     [{to_therapy:1,agency_transfer:0,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:2,agency_transfer:1,prior_matric:1,all_other:3},{to_therapy:2,agency_transfer:1,prior_matric:2,all_other:3}],
    'Southwest':     [{to_therapy:0,agency_transfer:0,prior_matric:1,all_other:1},{to_therapy:1,agency_transfer:0,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:2},{to_therapy:1,agency_transfer:1,prior_matric:1,all_other:3}],
  };
  Object.entries(relSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'release_dogs',r,f,v,true)));
  });

  // ── Therapy Dogs ──
  const thSeed = {
    'North Central': [{from_list:2,prog_or_prev:1},{from_list:2,prog_or_prev:1},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2}],
    'Northeast':     [{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2},{from_list:4,prog_or_prev:2},{from_list:4,prog_or_prev:3},{from_list:4,prog_or_prev:3}],
    'Northwest':     [{from_list:2,prog_or_prev:1},{from_list:2,prog_or_prev:1},{from_list:2,prog_or_prev:1},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2}],
    'South Central': [{from_list:2,prog_or_prev:1},{from_list:3,prog_or_prev:1},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2},{from_list:4,prog_or_prev:2}],
    'Southeast':     [{from_list:2,prog_or_prev:1},{from_list:2,prog_or_prev:1},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2}],
    'Southwest':     [{from_list:2,prog_or_prev:1},{from_list:2,prog_or_prev:1},{from_list:2,prog_or_prev:1},{from_list:3,prog_or_prev:2},{from_list:3,prog_or_prev:2}],
    'PSFO':          [{from_list:1,prog_or_prev:0},{from_list:1,prog_or_prev:0},{from_list:1,prog_or_prev:1},{from_list:1,prog_or_prev:1},{from_list:2,prog_or_prev:1}],
  };
  Object.entries(thSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'therapy_dogs',r,f,v,true)));
  });

  // ── Breeder Dogs ──
  const brSeed = {
    'North Central': [{selected_male:1,selected_female:2},{selected_male:1,selected_female:2},{selected_male:2,selected_female:2},{selected_male:2,selected_female:3},{selected_male:2,selected_female:3}],
    'Northeast':     [{selected_male:2,selected_female:3},{selected_male:2,selected_female:3},{selected_male:2,selected_female:3},{selected_male:3,selected_female:4},{selected_male:3,selected_female:4}],
    'Northwest':     [{selected_male:1,selected_female:1},{selected_male:1,selected_female:2},{selected_male:1,selected_female:2},{selected_male:2,selected_female:2},{selected_male:2,selected_female:2}],
    'South Central': [{selected_male:1,selected_female:2},{selected_male:2,selected_female:2},{selected_male:2,selected_female:3},{selected_male:2,selected_female:3},{selected_male:2,selected_female:3}],
    'Southeast':     [{selected_male:1,selected_female:2},{selected_male:1,selected_female:2},{selected_male:2,selected_female:2},{selected_male:2,selected_female:3},{selected_male:2,selected_female:3}],
    'Southwest':     [{selected_male:1,selected_female:1},{selected_male:1,selected_female:2},{selected_male:1,selected_female:2},{selected_male:2,selected_female:2},{selected_male:2,selected_female:2}],
  };
  Object.entries(brSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'breeder_dogs',r,f,v,true)));
  });
  // HQ breeder data
  const hqSeed = [{active_male:18,active_female:24,new_bc:6,repeat_bc:14,dams_bred:8,litters_whelped:6,live_puppies:38},{active_male:18,active_female:25,new_bc:7,repeat_bc:14,dams_bred:9,litters_whelped:7,live_puppies:42},{active_male:19,active_female:25,new_bc:7,repeat_bc:15,dams_bred:9,litters_whelped:7,live_puppies:44},{active_male:19,active_female:26,new_bc:8,repeat_bc:15,dams_bred:10,litters_whelped:8,live_puppies:48},{active_male:20,active_female:26,new_bc:8,repeat_bc:16,dams_bred:10,litters_whelped:8,live_puppies:50}];
  hqSeed.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'breeder_dogs','Headquarters',f,v,true)));

  // ── Puppy Placements ──
  const ppSeed = {
    'North Central': [{new_raiser:4,repeat_raiser:3,waitlist:18,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:17,matriculated:3},{new_raiser:5,repeat_raiser:4,waitlist:16,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:15,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:14,matriculated:4}],
    'Northeast':     [{new_raiser:5,repeat_raiser:4,waitlist:22,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:21,matriculated:4},{new_raiser:6,repeat_raiser:5,waitlist:20,matriculated:5},{new_raiser:6,repeat_raiser:5,waitlist:19,matriculated:5},{new_raiser:6,repeat_raiser:5,waitlist:18,matriculated:5}],
    'Northwest':     [{new_raiser:3,repeat_raiser:2,waitlist:14,matriculated:2},{new_raiser:3,repeat_raiser:3,waitlist:13,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:13,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:12,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:11,matriculated:3}],
    'South Central': [{new_raiser:4,repeat_raiser:3,waitlist:19,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:18,matriculated:3},{new_raiser:5,repeat_raiser:4,waitlist:17,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:16,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:15,matriculated:4}],
    'Southeast':     [{new_raiser:4,repeat_raiser:3,waitlist:17,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:16,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:16,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:15,matriculated:4},{new_raiser:5,repeat_raiser:4,waitlist:14,matriculated:4}],
    'Southwest':     [{new_raiser:3,repeat_raiser:2,waitlist:15,matriculated:2},{new_raiser:3,repeat_raiser:3,waitlist:14,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:13,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:13,matriculated:3},{new_raiser:4,repeat_raiser:3,waitlist:12,matriculated:3}],
  };
  Object.entries(ppSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'puppy_placements',r,f,v,true)));
  });
  // PSFO puppy placements
  const psfoSeed = [{puppy_classes:3,raiser_followup:8},{puppy_classes:3,raiser_followup:9},{puppy_classes:4,raiser_followup:10},{puppy_classes:4,raiser_followup:11},{puppy_classes:4,raiser_followup:11}];
  psfoSeed.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'puppy_placements','PSFO',f,v,true)));

  // ── Active Puppy Raisers ──
  const arSeed = {'North Central':[148,151,154,157,160],'Northeast':[182,185,188,192,195],'Northwest':[124,126,129,131,134],'South Central':[156,159,162,165,168],'Southeast':[143,146,149,152,155],'Southwest':[130,133,136,139,142]};
  Object.entries(arSeed).forEach(([r,months])=>{
    months.forEach((v,i)=>sv(2026,i+1,'active_raisers',r,'total_raisers',v,true));
  });

  // ── Puppy Interest ──
  const piSeed = {
    'North Central': [{app_requests:12,apps_received:8,apps_approved:6,proj_waitlist:22,fostered:2},{app_requests:13,apps_received:9,apps_approved:7,proj_waitlist:21,fostered:2},{app_requests:14,apps_received:10,apps_approved:8,proj_waitlist:20,fostered:3},{app_requests:14,apps_received:10,apps_approved:8,proj_waitlist:19,fostered:3},{app_requests:15,apps_received:11,apps_approved:9,proj_waitlist:18,fostered:3}],
    'Northeast':     [{app_requests:15,apps_received:11,apps_approved:9,proj_waitlist:28,fostered:3},{app_requests:16,apps_received:12,apps_approved:10,proj_waitlist:27,fostered:3},{app_requests:17,apps_received:12,apps_approved:10,proj_waitlist:26,fostered:4},{app_requests:18,apps_received:13,apps_approved:11,proj_waitlist:25,fostered:4},{app_requests:18,apps_received:13,apps_approved:11,proj_waitlist:24,fostered:4}],
    'Northwest':     [{app_requests:10,apps_received:7,apps_approved:5,proj_waitlist:18,fostered:2},{app_requests:10,apps_received:7,apps_approved:6,proj_waitlist:17,fostered:2},{app_requests:11,apps_received:8,apps_approved:6,proj_waitlist:17,fostered:2},{app_requests:12,apps_received:8,apps_approved:7,proj_waitlist:16,fostered:2},{app_requests:12,apps_received:9,apps_approved:7,proj_waitlist:15,fostered:3}],
    'South Central': [{app_requests:13,apps_received:9,apps_approved:7,proj_waitlist:24,fostered:2},{app_requests:14,apps_received:10,apps_approved:8,proj_waitlist:23,fostered:3},{app_requests:14,apps_received:10,apps_approved:8,proj_waitlist:22,fostered:3},{app_requests:15,apps_received:11,apps_approved:9,proj_waitlist:21,fostered:3},{app_requests:16,apps_received:12,apps_approved:9,proj_waitlist:20,fostered:3}],
    'Southeast':     [{app_requests:12,apps_received:8,apps_approved:6,proj_waitlist:21,fostered:2},{app_requests:13,apps_received:9,apps_approved:7,proj_waitlist:20,fostered:2},{app_requests:13,apps_received:9,apps_approved:8,proj_waitlist:20,fostered:3},{app_requests:14,apps_received:10,apps_approved:8,proj_waitlist:19,fostered:3},{app_requests:15,apps_received:11,apps_approved:9,proj_waitlist:18,fostered:3}],
    'Southwest':     [{app_requests:11,apps_received:7,apps_approved:6,proj_waitlist:19,fostered:2},{app_requests:11,apps_received:8,apps_approved:6,proj_waitlist:18,fostered:2},{app_requests:12,apps_received:8,apps_approved:7,proj_waitlist:18,fostered:2},{app_requests:13,apps_received:9,apps_approved:8,proj_waitlist:17,fostered:3},{app_requests:13,apps_received:10,apps_approved:8,proj_waitlist:16,fostered:3}],
  };
  Object.entries(piSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'puppy_interest',r,f,v,true)));
  });

  // ── Prison Programs ──
  const prSeed = {
    'North Central': [{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1},{active_programs:4,active_puppies:5,placed_month:1},{active_programs:4,active_puppies:5,placed_month:2},{active_programs:4,active_puppies:6,placed_month:2}],
    'Northeast':     [{active_programs:4,active_puppies:5,placed_month:1},{active_programs:4,active_puppies:6,placed_month:2},{active_programs:5,active_puppies:6,placed_month:2},{active_programs:5,active_puppies:7,placed_month:2},{active_programs:5,active_puppies:7,placed_month:2}],
    'Northwest':     [{active_programs:2,active_puppies:3,placed_month:1},{active_programs:2,active_puppies:3,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1}],
    'South Central': [{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:5,placed_month:1},{active_programs:4,active_puppies:5,placed_month:2},{active_programs:4,active_puppies:5,placed_month:2},{active_programs:4,active_puppies:6,placed_month:2}],
    'Southeast':     [{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:5,placed_month:1},{active_programs:4,active_puppies:5,placed_month:2},{active_programs:4,active_puppies:5,placed_month:2}],
    'Southwest':     [{active_programs:2,active_puppies:3,placed_month:1},{active_programs:2,active_puppies:3,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:4,placed_month:1},{active_programs:3,active_puppies:4,placed_month:2}],
  };
  Object.entries(prSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'prison_programs',r,f,v,true)));
  });

  // ── College Clubs ──
  const ccSeed = {
    'North Central': [{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:2}],
    'Northeast':     [{active_clubs:3,active_puppies:4,placed_month:1},{active_clubs:3,active_puppies:5,placed_month:1},{active_clubs:4,active_puppies:5,placed_month:2},{active_clubs:4,active_puppies:6,placed_month:2},{active_clubs:4,active_puppies:6,placed_month:2}],
    'Northwest':     [{active_clubs:2,active_puppies:2,placed_month:0},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:3,active_puppies:3,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:1}],
    'South Central': [{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:1},{active_clubs:3,active_puppies:5,placed_month:2}],
    'Southeast':     [{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:1},{active_clubs:3,active_puppies:4,placed_month:2}],
    'Southwest':     [{active_clubs:1,active_puppies:2,placed_month:0},{active_clubs:2,active_puppies:2,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:2,active_puppies:3,placed_month:1},{active_clubs:3,active_puppies:3,placed_month:1}],
  };
  Object.entries(ccSeed).forEach(([r,months])=>{
    months.forEach((d,i)=>Object.entries(d).forEach(([f,v])=>sv(2026,i+1,'college_clubs',r,f,v,true)));
  });

  // ── Graduate Log — pre-populated manual entries ──
  manualEntries = [
    {id:1,first:'Sarah',last:'Martinez',mi:'L',dog:'Apollo',cat:'Dog Placement',region:'Northeast',date:'2026-01-15',matchClient:null,matchDog:null},
    {id:2,first:'James',last:'Thompson',mi:'R',dog:'Bella',cat:'Service',region:'Northwest',date:'2026-01-22',matchClient:null,matchDog:null},
    {id:3,first:'Emily',last:'Chen',mi:'',dog:'Ranger',cat:'Dog Placement',region:'South Central',date:'2026-02-08',matchClient:null,matchDog:null},
    {id:4,first:'Michael',last:'Williams',mi:'T',dog:'Luna',cat:'Hearing',region:'Northeast',date:'2026-02-14',matchClient:null,matchDog:null},
    {id:5,first:'Amanda',last:'Rodriguez',mi:'K',dog:'Cody',cat:'Dog Placement',region:'Southeast',date:'2026-03-03',matchClient:null,matchDog:null},
    {id:6,first:'David',last:'Johnson',mi:'',dog:'Sadie',cat:'Facility',region:'North Central',date:'2026-03-18',matchClient:null,matchDog:null},
    {id:7,first:'Jessica',last:'Brown',mi:'M',dog:'Tucker',cat:'Dog Placement',region:'Southwest',date:'2026-04-05',matchClient:null,matchDog:null},
    {id:8,first:'Robert',last:'Davis',mi:'A',dog:'Molly',cat:'PTSD',region:'Northeast',date:'2026-04-12',matchClient:null,matchDog:null},
    {id:9,first:'Lauren',last:'Wilson',mi:'',dog:'Brody',cat:'Dog Placement',region:'Northwest',date:'2026-05-02',matchClient:null,matchDog:null},
    {id:10,first:'Kevin',last:'Garcia',mi:'J',dog:'Daisy',cat:'Medical Alert',region:'South Central',date:'2026-05-09',matchClient:null,matchDog:null},
  ];
...
(async function init(){
  // Step 1: check localStorage for an existing session immediately.
  // This is synchronous-ish and resolves before any auth events fire.
  const { data: { session } } = await SB.auth.getSession();

  if(session){
    // Valid session found — restore without showing login at all
  ...
  // Step 2: listen for real auth changes AFTER the initial check.
  // Use a flag to ignore the spurious SIGNED_OUT that fires on page
  // load before localStorage session is restored.
  const _loadedAt = Date.now();

  SB.auth.onAuthStateChange(async (event, session) => {
    // Ignore SIGNED_OUT events within 3 seconds of page load —
    // these are always false positives from Supabase's init sequence.
    if(event === 'SIGNED_OUT'){
      if(Date.now() - _loadedAt < 3000) return;
      _currentUser = null;
      _loginComplete = false;
      showLogin();
      return;
    ...
    if((event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') && session){
      if(_loginComplete) return; // already loaded
      _currentUser = session.user;
      await afterLogin();
    }
  });
