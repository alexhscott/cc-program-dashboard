...
function cacheDataLocally(){
  try{ localStorage.setItem('cc-data-cache', JSON.stringify(appData)); } catch(e){}
}

function loadCachedData(){
  try{
    const raw = localStorage.getItem('cc-data-cache');
    if(!raw) return false;
    const parsed = JSON.parse(raw);
    if(!parsed || typeof parsed !== 'object') return false;
    Object.keys(parsed).forEach(function(yr){
      Object.keys(parsed[yr]).forEach(function(mo){
        Object.keys(parsed[yr][mo]).forEach(function(sec){
          Object.keys(parsed[yr][mo][sec]).forEach(function(region){
            Object.keys(parsed[yr][mo][sec][region]).forEach(function(field){
              sv(+yr,+mo,sec,region,field,parsed[yr][mo][sec][region][field],true);
            });
          });
        });
      });
